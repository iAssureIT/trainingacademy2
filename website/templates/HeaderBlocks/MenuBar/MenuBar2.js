...
function MenuBar2(props) {
	...
	useEffect(() => {
		...
		if (window.location.pathname === '/privacy-policy/' || window.location.pathname === '/genrated-leads/') {
		...
		for (var i = 0; i < links.length; i++) {
			if (window.location.href == links[i].href) {
				var pId = links[i].parentNode.getAttribute('data-servicepage_id');
				// console.log("pID", pId)
			...
			<div id={"Menubar"} ref={menuRef} className=" fixed z-10 w-full ">
				<nav className="navBar1 bg-transparent border-gray-200  ">
					<div className={props?.inputData?.navCss ? props?.inputData?.navCss : "max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4"}>
						...
						<button id="navbar-toggle" data-collapse-toggle="navbar-dropdown"  className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-200 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 " aria-controls="navbar-dropdown" aria-expanded="false">
						...
						<div className="lg:grid lg:grid-cols-1 w-full md:w-auto">
							...
							<div className="hidden w-full md:block md:w-auto " id="navbar-dropdown">
								<ul className="flex flex-col font-medium md:py-4 md:p-0 mt-4 border border-gray-100 md:rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0 text-black md:text-white  bg-white md:bg-transparent">
													...
													<li key={index} >
														{
															currentLink == data.link
																...
																:
																<button id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar" className="flex relative items-center justify-between w-full py-2 pl-3 pr-4 text-sm lg:text-lg font-bold text-gray-700 md:text-white border-b border-gray-100 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:p-0 md:w-auto  ">
																	{data.label}
																	<svg className="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
																		<path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 4 4 4-4" />
																	</svg>
														...
														<div id="dropdownNavbar" className="absolute z-10 md:left-0 w-auto xs:w-44 sm:w-72 md:w-full lg:w-full hidden font-normal md:mt-7 bg-white divide-y text-lg divide-gray-100 md:rounded-lg shadow-[0_3px_10px_rgb(0,0,0,0.2)] ">
															<div className=" hidden md:block w-full absolute right-[19rem] xl:right-[26rem] 2xl:right-[30rem] -top-5">
															...
															</div> <ul className="py-4 text-sm text-gray-700  w-[280px] sm:w-44 md:w-52 lg:w-44 " >

																{data?.subMenu?.map((val, index) => {
																	return (
																		// <li>
																		// <a href="#" className="block px-4 py-2 hover:bg-gray-100 ">{val.submenutitle}</a>
																		// </li>

																		<li key={index} className="w-full sm:w-72 md:w-full lg:w-32 cursor-pointer" >
																			<div id={val.id} data-parent={val.id} data-dropdown-placement="right-start"  className=" flex flex-row lg:text-[14px] w-full md:w-full lg:w-80 2xl:w-[26rem] md:my-2 text-left font-bold text-sm py-2 items-center justify-between md:justify-start px-4 hover:bg-gray-100  doubleDropdownButton">
																				<div data-parent={val.id} className=" bg-gray-400 rounded mr-4">
																					<img id={'menu-img-' + val.id} data-parent={val.id} src={val.img} className='menu-img' alt={"menu-img"} />
																				</div>
																			...
																			<div id={"doubleDropdown" + val.id} className="z-10 absolute hidden md:top-5 -left-44 sm:left-1 md:left-10 lg:left-40 2xl:left-72 w-full px-2 sm:w-[30rem] md:w-[34rem] lg:w-[45rem] xl:w-3/4 xxl:w-4/5 bg-white divide-y divide-gray-100 rounded-lg shadow-[0_3px_10px_rgb(0,0,0,0.2)]  transform translate-x-44 dd ">
																				<div className={props?.inputData?.gridCss ? props?.inputData?.gridCss : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-10"}>
																					<ul className="py-2 md:py-0 text-sm text-gray-700  lg:col-span-2 xl:col-span-2 xxl:!col-span-1">
																						{val?.NestedMenu?.map((data, ind) => {
																							return (
																								<li key={ind}>
																									{/* <a href="#" className="block px-4 py-2 hover:bg-gray-100 ">{data.title}</a> */}
																									<div className={props.inputData?.repeatedBlkCss ? props.inputData?.repeatedBlkCss : ' my-10 flex shadow-xl sm:h-36 md:h-32 py-5'}>
																										...
																										<div className='text-left my-auto w-full'>
																											<div data-servicepage_id={val.id} className={props?.inputData?.titleCss ? props?.inputData?.titleCss : "font-bold text-base sm:text-lg mb-2"}>
													...
													<li key={index} title={data?.title} >
														{
															...
															currentLink == data.link
																?
																<a href={data.link} className=" menuItemC mainMenuActive block py-2 pl-3 pr-4 text-black md:text-white text-sm md:text-lg rounded md:bg-transparent font-bold md:p-0 " aria-current="page">{data.label}</a>
																:
																<a href={data.link} className=" menuItemC block py-2 pl-3 pr-4 text-black md:text-white text-sm lg:text-lg rounded md:bg-transparent font-bold md:p-0 " aria-current="page">{data.label}</a>
