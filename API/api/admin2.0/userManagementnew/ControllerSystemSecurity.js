...
exports.fetch_user_ID = (req, res, next) => {
  User.findOne({ _id: req.params.ID })
    .select(
      "profile.firstname profile.lastname profile.status profile.fullName roles profile.emailId profile.mobNumber"
    )
    .exec()
    .then((data) => {
      if (data) {
...
exports.fetch_users = (req, res, next) => {
  ...
  User.find({})
    .select(
      "profile.firstname profile.lastname profile.status profile.fullName roles profile.emailId profile.mobNumber profile.clientId"
    )
    .sort({ createdAt: -1 })
    // .skip(startRange)
    // .limit(limitRange)
