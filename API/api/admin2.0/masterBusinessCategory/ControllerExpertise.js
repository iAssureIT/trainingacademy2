    ...
    BusinessCategory.find({_id : req.body.dropdownID})
                    .then(catgData=>{
                        if(catgData && catgData.length>0){
                            ...
                            var duplicateExp = expertise1.some(item => expertise2.includes(item))
                            if(duplicateExp){
                                res.status(200).json({created : false, message: "Expertise already exists in this Category"})
                            }else{
                                if(expertise1){
...
exports.deleteBusinessCategory = (req, res, next)=>{
    BusinessCategory.deleteOne({_id: req.params.fieldID})
        .exec()
        .then(data=>{
            console.log("data deleteBusinessCategory",data);
